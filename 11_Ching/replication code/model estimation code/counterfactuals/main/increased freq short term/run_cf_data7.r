  ...
  for(st in stids) {
    for(y in 1:7) {
      ...
      if(sum(inds & prdf[[prednames[j]]] != -1) > 0) {
        nsale = sum(inds & prdf[[prednames[j]]] == 1)
        obs = which(inds & prdf[[prednames[j]]] == 0)
        n1 = paste("AVN",prednames[[j]],sep="")
        n2 = paste("AVD",prednames[[j]],sep="")
        meandf = mean( (prdf[[n1]][inds]-prdf[[n2]][inds])/prdf[[n1]][inds], na.rm = TRUE )
        if(nsale < length(obs)) {
          obschange = sample(obs,nsale)
          prdf[[prod]][obschange] = prdf[[prod]][obschange]*(1-meandf)
          chobs[obschange,j] = TRUE
        } else {
          prdf[[prod]][obs] = prdf[[prod]][obs]*(1-meandf)
          chobs[obs,j] = TRUE
        }
        ...
        if(is.finite(temp) & temp > 0) {
          nsale = sum(inds & dealprice[,j])
          obs = which(inds & !dealprice[,j])
          n1 = paste("AVN",prednames[[j]],sep="")
          n2 = paste("AVD",prednames[[j]],sep="")
          meandf = mean( (prdf[[n1]][inds]-prdf[[n2]][inds])/prdf[[n1]][inds], na.rm = TRUE )
          if(nsale < length(obs)) {
...
load(paste("calculon_runs/est",spec.index,"_mcmcout.RData",sep=""))
...
load(paste("calculon_runs/estml",spec.index,".RData",sep=""))
