...
mininv.new1 = apply(inv.new1$invbig[,keep1],2,min,na.rm=TRUE)
invqu = quantile(mininv.new1,probs=seq(0,1,1/nclust))

if(1==1) {

  crclust = hclust(dist(hhcrate),method="ward.D2")

  crassign = cutree(crclust,nclust)

  idkeep = allids[keep1]  #missing brindex 4, 14, I think is ok

  idkeep = idkeep[crassign==clust.index]
  hhcrate = hhcrate[crassign==clust.index]

} else {

  itype = rep(0,sum(keep1))

  for(i in 2:(nclust+1)) {
    if(invqu[i-1] == invqu[i]) {
      itype[mininv.new1==invqu[i]] = i-1
    } else {
      if(i==2) {
        itype[mininv.new1 >= invqu[i-1] & mininv.new1 <= invqu[i]] = i-1
      } else {
        itype[mininv.new1 > invqu[i-1] & mininv.new1 <= invqu[i]] = i-1
      }
    }
  ...
  idkeep = idkeep[itype==clust.index]
  hhcrate = hhcrate[itype==clust.index]
...
contmodel = as.integer(contmodel), debug = as.integer(2), idrawtype = as.integer(idrawtype),
...
#Mcmc$fixedcoef[c(3:12,19,21,22)] = FALSE
